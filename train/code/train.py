...
from sklearn.metrics import f1_score, roc_auc_score, accuracy_score, precision_recall_fscore_support, recall_score, precision_score
...
#         logger.info(f"tokenizer: {batch["ABSTRACT"]}")
    ...
    try:
        outputs = model(input_ids=train_dataset['input_ids'][0].unsqueeze(0), labels=train_dataset['labels'][0].unsqueeze(0))
